    ...
    const int flags = ARCHIVE_EXTRACT_TIME)
{
  ...
  // Set up a lambda for common error handling when calling libarchive.
  //
  // If function returns true, some fatal error occurred.
  // If function returns false, warning (or no error) occurred, ok to continue.
  //
  // When warning or error occurs, reason for error is logged,
  // and returnStatus is modified to return some sort of an error.

  auto handleError = [&returnStatus](int result, struct archive *a) {
  ...
  while (true) {
    ...
    if (handleError(result, reader.get())) {
      return returnStatus;
    }
    ...
    if (handleError(result, writer.get())) {
      return returnStatus;
    }
