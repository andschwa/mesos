...
//
// NOTE: We default to close-on-exec behavior (no in heritance) because we never
// use inheritable sockets. Overlapped I/O is enabled to match the default
// behavior of `::socket`.
...
inline os::SocketFD accept(
    const os::SocketFD& fd, sockaddr* addr, socklen_t* addrlen)
{
...
inline int bind(
    const os::SocketFD& fd, const sockaddr* addr, socklen_t addrlen)
{
...
inline int connect(
    const os::SocketFD& fd, const sockaddr* address, socklen_t addrlen)
{
  CHECK_LE(addrlen, INT32_MAX);
...
inline ssize_t send(
    const os::SocketFD& fd, const void* buf, size_t len, int flags)
{
...
inline ssize_t recv(const os::SocketFD& fd, void* buf, size_t len, int flags)
{
  CHECK_LE(len, INT32_MAX);
