...
{
  // These comparisons must come first because a default-constructed `WindowsFD`
  // is invalid, but has no type, and a `WindowFD` constructed from `-1` is
  // invalid, but should be comparable against both `HANDLE` and `SOCKET` types.
...
inline bool operator==(const WindowsFD& left, const WindowsFD& right)
{
  // This is `false` because an invalid `HANDLE` and an invalid
  // `SOCKET` are still not equal.
