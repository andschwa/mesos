...
inline Try<off_t> lseek(int_fd fd, off_t offset, int whence)
{
  ...
  // `FILE_END`. Thus we don't need to map them.

  LARGE_INTEGER old_offset;
  old_offset.QuadPart = offset;
  ...
  const BOOL result =
    ::SetFilePointerEx(fd, old_offset, &new_offset, static_cast<DWORD>(whence));
